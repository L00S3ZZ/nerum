...
class UserIntegration(Base):
    ...
    integration_type = Column(String, index=True, nullable=False)
...
try:
    with engine.connect() as conn:
        ...
        conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS terms_accepted_at TIMESTAMP NULL"))
        conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS terms_version VARCHAR DEFAULT 'v1.0'"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS ix_user_integrations_user_type ON user_integrations (user_id, integration_type)"))
        conn.commit()
