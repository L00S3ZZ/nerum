...
const BUSINESS_TEMPLATES = {
  ...
  clinic: {
    ...
    statusColors: { 'Scheduled': '#60a5fa', 'Completed': '#34d399', 'Cancelled': '#ff8a7a', 'No Show': '#fbbf24' },
    use_cases: [
  ...
  shop: {
    ...
    statusColors: { 'Pending': '#fbbf24', 'Paid': '#34d399', 'Shipped': '#2AABEE', 'Delivered': '#34d399', 'Cancelled': '#ff8a7a' },
    use_cases: [
  ...
  restaurant: {
    ...
    statusColors: { 'Pending': '#fbbf24', 'Preparing': '#fb923c', 'Ready': '#34d399', 'Delivered': '#2AABEE', 'Cancelled': '#ff8a7a' },
    use_cases: [
  ...
  realestate: {
    ...
    statusColors: { 'Pending': '#fbbf24', 'Paid': '#34d399', 'Overdue': '#ff8a7a', 'Scheduled': '#60a5fa' },
    use_cases: [
  ...
  gym: {
    ...
    statusColors: { 'Active': '#34d399', 'Expiring': '#fbbf24', 'Expired': '#ff8a7a', 'Paused': '#34d399' },
    use_cases: [
  ...
  company: {
    ...
    statusColors: { 'Pending': '#fbbf24', 'In Progress': '#34d399', 'Completed': '#34d399', 'Overdue': '#ff8a7a' },
    use_cases: [
  ...
  custom: {
    ...
    statusColors: { 'Pending': '#fbbf24', 'Done': '#34d399', 'Skip': '#94a3b8', 'Active': '#34d399' },
    use_cases: [
...
async function showSmartLists() {
  ...
  const border = isDark ? 'rgba(255,255,255,0.08)' : 'rgba(52,211,153,0.15)';
  const color = isDark ? '#fff' : '#000000';
  const muted = isDark ? 'rgba(255,255,255,0.4)' : '#34d399';
...
async function renderSmartListsHome(page) {
  ...
  page.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
    <div>
      ...
      <div style="font-size:12px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-top:2px">Auto-send messages based on your data</div>
    </div>
    <button onclick="showCreateListModal()" style="padding:10px 20px;border-radius:12px;background:linear-gradient(135deg,#34d399,#2dd4bf);border:none;color:#fff;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">+ New List</button>
  </div>
  ...
  try {
    ...
    if (lists.length === 0) {
      container.innerHTML = `
        <div style="text-align:center;padding:60px 20px;background:${isDark?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.5)'};border-radius:20px;border:1px dashed ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'}">
          <div style="font-size:48px;margin-bottom:16px">📋</div>
          <div style="font-size:16px;font-weight:700;color:${isDark?'#fff':'#000000'};margin-bottom:8px">No Smart Lists yet</div>
          <div style="font-size:13px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:24px">Create a list for your school, clinic, shop or any business</div>
          <button onclick="showCreateListModal()" style="padding:12px 28px;border-radius:12px;background:linear-gradient(135deg,#34d399,#2dd4bf);border:none;color:#fff;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">Create First List →</button>
        </div>`;
      return;
    }
    ...
    container.innerHTML = `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px">
      ${lists.map(l => {
        ...
        return `
        <div style="background:${isDark?'rgba(255,255,255,0.04)':'rgba(255,255,255,0.7)'};border:1px solid ${isDark?'rgba(255,255,255,0.08)':'rgba(52,211,153,0.15)'};border-radius:16px;padding:20px;cursor:pointer;transition:all 0.2s"
          onmouseover="this.style.transform='translateY(-2px)';this.style.borderColor='rgba(52,211,153,0.3)'"
          onmouseout="this.style.transform='';this.style.borderColor='${isDark?'rgba(255,255,255,0.08)':'rgba(52,211,153,0.15)'}'"
          ...
          <div style="font-size:11px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:14px">${tmpl.label.substring(3)} · ${l.schedule_time} daily</div>
          <div style="display:flex;justify-content:space-between;margin-bottom:8px">
            ...
            <span style="font-size:11px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'}">Total: ${total}</span>
          </div>
          <div style="height:4px;border-radius:10px;background:${isDark?'rgba(255,255,255,0.08)':'rgba(52,211,153,0.1)'}">
            <div style="height:100%;border-radius:10px;background:linear-gradient(90deg,#34d399,#2dd4bf);width:${pct}%"></div>
          </div>
          <div style="display:flex;justify-content:space-between;margin-top:12px">
            <span style="font-size:10px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'}">Last run: ${l.last_run ? new Date(l.last_run).toLocaleDateString() : 'Never'}</span>
  ...
  try {
    ...
    page.innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
        <button onclick="showSmartLists()" style="padding:8px 16px;border-radius:10px;background:${isDark?'rgba(255,255,255,0.06)':'rgba(52,211,153,0.08)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'rgba(255,255,255,0.6)':'#34d399'};font-size:12px;cursor:pointer;font-family:inherit">← Back</button>
        <div style="flex:1">
          <div style="font-size:18px;font-weight:800;color:${isDark?'#fff':'#000000'}">${tmpl.label.split(' ')[0]} ${lst.name}</div>
          <div style="font-size:11px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'}">${tmpl.label.substring(3)} · Auto-sends at ${lst.schedule_time} daily</div>
        </div>
        <div style="display:flex;gap:8px">
          ...
          <button onclick="showAddRecordModal(${listId})" style="padding:10px 20px;border-radius:12px;background:linear-gradient(135deg,#34d399,#2dd4bf);border:none;color:#fff;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">+ Add Row</button>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:20px">
        ...
        <div style="background:${isDark?'rgba(52,211,153,0.08)':'rgba(52,211,153,0.05)'};border:1px solid rgba(52,211,153,0.15);border-radius:12px;padding:14px;text-align:center">
          <div style="font-size:22px;font-weight:800;color:#34d399">${records.length}</div>
          <div style="font-size:10px;color:#34d399;margin-top:2px">Total</div>
        </div>
        <div style="background:${isDark?'rgba(52,211,153,0.08)':'rgba(52,211,153,0.05)'};border:1px solid rgba(52,211,153,0.15);border-radius:12px;padding:14px;text-align:center">
          <div style="font-size:22px;font-weight:800;color:#34d399">${records.reduce((s,r)=>s+r.message_count,0)}</div>
          <div style="font-size:10px;color:#34d399;margin-top:2px">Messages Sent</div>
        </div>
      </div>

      <!-- Table -->
      <div style="background:${isDark?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.7)'};border:1px solid ${isDark?'rgba(255,255,255,0.08)':'rgba(52,211,153,0.15)'};border-radius:16px;overflow:hidden">
        <div style="overflow-x:auto">
          <table style="width:100%;border-collapse:collapse">
            <thead>
              <tr style="border-bottom:1px solid ${isDark?'rgba(255,255,255,0.08)':'rgba(52,211,153,0.1)'}">
                <th style="padding:12px 14px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'};font-weight:600">Name</th>
                <th style="padding:12px 14px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'};font-weight:600">Phone</th>
                ${columns.map(col => `<th style="padding:12px 14px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'};font-weight:600">${tmpl.columnLabels[col] || col}</th>`).join('')}
                <th style="padding:12px 14px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'};font-weight:600">Status</th>
                <th style="padding:12px 14px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'};font-weight:600">Last Sent</th>
                <th style="padding:12px 14px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'};font-weight:600">Actions</th>
              </tr>
            </thead>
            <tbody id="records-tbody">
              ${records.length === 0 ? `
                ...
                records.map(r => {
                  ...
                  const statusColor = tmpl.statusColors[r.status] || '#34d399';
                  return `<tr style="border-bottom:1px solid ${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.06)'};transition:background 0.15s"
                    onmouseover="this.style.background='${isDark?'rgba(255,255,255,0.02)':'rgba(52,211,153,0.03)'}'"
                    onmouseout="this.style.background=''">
                    <td style="padding:12px 14px;font-size:12px;font-weight:600;color:${isDark?'#fff':'#000000'}">${r.name}</td>
                    <td style="padding:12px 14px;font-size:12px;color:${isDark?'rgba(255,255,255,0.6)':'#34d399'}">${r.phone}</td>
                    ${columns.map(col => `<td style="padding:12px 14px;font-size:12px;color:${isDark?'rgba(255,255,255,0.6)':'#34d399'}">${fields[col] || '—'}</td>`).join('')}
                    ...
                    <td style="padding:12px 14px;font-size:11px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'}">${r.last_message_sent ? new Date(r.last_message_sent).toLocaleDateString() + ' ' + new Date(r.last_message_sent).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}) : '—'}</td>
                    <td style="padding:12px 14px">
                      <div style="display:flex;gap:6px">
                        <button onclick="showEditRecordModal(${r.id})" style="padding:4px 10px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.2);color:#34d399;font-size:10px;cursor:pointer;font-family:inherit">Edit</button>
...
function showCreateListModal() {
  ...
  modal.innerHTML = `
    <div style="width:100%;max-width:520px;margin:20px;background:${isDark?'#000000':'#fff'};border:1px solid rgba(52,211,153,0.2);border-radius:20px;padding:28px;max-height:85vh;overflow-y:auto;animation:slideUp 0.3s ease">
      ...
      <div style="margin-bottom:16px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:8px;font-weight:600">What type of business?</div>
        <select id="cl-business-type" onchange="updateUseCases()" style="width:100%;padding:12px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:13px;outline:none;font-family:inherit">
          ${businessOptions}
        </select>
      </div>
      ...
      <!-- Step 2: Use Case -->
      <div style="margin-bottom:16px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:8px;font-weight:600">What do you want to send?</div>
        <select id="cl-use-case" onchange="updateMessageTemplate()" style="width:100%;padding:12px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:13px;outline:none;font-family:inherit">
        </select>
      </div>

      <!-- List Name -->
      <div style="margin-bottom:16px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:8px;font-weight:600">List Name</div>
        <input id="cl-name" placeholder="e.g. Class 8 Fee Tracker" style="width:100%;padding:12px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:13px;outline:none;font-family:inherit"/>
      </div>

      <!-- Message Template -->
      <div style="margin-bottom:16px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:8px;font-weight:600">Message Template</div>
        <textarea id="cl-message" rows="4" style="width:100%;padding:12px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:12px;outline:none;font-family:inherit;resize:none;line-height:1.6"></textarea>
        <div style="font-size:10px;color:${isDark?'rgba(255,255,255,0.3)':'#34d399'};margin-top:4px">Use {name}, {phone} and column names in curly braces</div>
      </div>

      <!-- Schedule -->
      <div style="margin-bottom:16px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:8px;font-weight:600">Auto-send time (daily)</div>
        <input id="cl-time" type="time" value="17:00" style="width:100%;padding:12px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:13px;outline:none;font-family:inherit"/>
      </div>

      <!-- Send via -->
      <div style="margin-bottom:24px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:8px;font-weight:600">Send via</div>
        <div style="display:flex;gap:10px">
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:12px;color:${isDark?'rgba(255,255,255,0.6)':'#34d399'}">
      ...
      <button onclick="createList()" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#34d399,#2dd4bf);border:none;color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit">
        Create Smart List →
      </button>
    </div>`;
...
function showAddRecordModal(listId) {
  ...
  modal.innerHTML = `
    <div style="width:100%;max-width:480px;margin:20px;background:${isDark?'#000000':'#fff'};border:1px solid rgba(52,211,153,0.2);border-radius:20px;padding:28px;max-height:85vh;overflow-y:auto;animation:slideUp 0.3s ease">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px">
      ...
      <div style="display:flex;flex-direction:column;gap:12px">
        <div>
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:6px;font-weight:600">Name *</div>
          <input id="ar-name" placeholder="Full name" style="width:100%;padding:11px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:13px;outline:none;font-family:inherit"/>
        </div>
        <div>
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:6px;font-weight:600">WhatsApp Number *</div>
          <input id="ar-phone" placeholder="+91 98765 43210" style="width:100%;padding:11px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:13px;outline:none;font-family:inherit"/>
        </div>
        ${columns.map(col => `
          <div>
            <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:${isDark?'rgba(255,255,255,0.4)':'#34d399'};margin-bottom:6px;font-weight:600">${tmpl.columnLabels[col] || col}</div>
            ${col.endsWith('_status') || col === 'status' || col === 'reminded' ? `
              <select id="ar-${col}" style="width:100%;padding:11px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:13px;outline:none;font-family:inherit">
                ${tmpl.statusOptions.map(s => `<option value="${s}">${s}</option>`).join('')}
              </select>` : `
              <input id="ar-${col}" placeholder="${tmpl.columnLabels[col] || col}" style="width:100%;padding:11px 14px;border-radius:12px;background:${isDark?'rgba(255,255,255,0.05)':'rgba(52,211,153,0.05)'};border:1px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(52,211,153,0.2)'};color:${isDark?'#fff':'#000000'};font-size:13px;outline:none;font-family:inherit"/>`}
          </div>
        `).join('')}
      </div>
      <button onclick="saveRecord(${listId})" style="width:100%;padding:13px;border-radius:12px;background:linear-gradient(135deg,#34d399,#2dd4bf);border:none;color:#fff;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;margin-top:20px">
        Add Row ✓
      </button>
    </div>`;
